...
// Computes the K-nearest neighbors inside a radius of p_i in a point set points
std::vector<size_t> CommonOperations::getNN_in_radius(glm::vec3 p_i, float radius) {
	std::vector<std::pair<size_t, float>> neighbors;
	nn.getNeighborsInRadius(p_i, radius, neighbors);
